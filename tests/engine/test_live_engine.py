...
def test_live_evidence_update_is_journaled_before_broker_application(monkeypatch):
    monkeypatch.setattr(
        "koval.engine.live_engine.assemble_from_graph", lambda graph: _AlwaysLongStrategy()
    )
    records = []

    class EvidenceBroker(_RecordingBroker):
        target = "paper"

        def validate_execution_evidence_update(self, update):
            return True

        def apply_execution_evidence_update(self, update):
            assert records[-1]["kind"] == "execution_evidence_update"
            self.applied = update
            return True
    ...
    update = decode_execution_evidence_update(
        {
            "update_id": "binance:BTCUSDT:0",
            "timestamp_ms": 0,
            "mark_price": {"timestamp_ms": 0, "price": "100", "source": "test"},
        }
    )

    assert engine.apply_execution_evidence_update(update) is True
    assert broker.applied == update
    assert records[-1]["payload"] == update.as_config()
...
def test_observed_quote_paper_ignores_late_bar_open_and_journals_causal_fill(monkeypatch):
    monkeypatch.setattr(
        "koval.engine.live_engine.assemble_from_graph", lambda graph: _SingleEntryStrategy()
    )
    records = []
    ...
    engine = LiveEngine(
        ...
        session_id="causal-test",
        on_record=records.append,
        on_status=statuses.append,
        on_event=events.append,
    )
    engine._process_bar(np.array([0, 100.0, 101.0, 99.0, 100.0, 10.0]))
    intent = next(record for record in records if record["kind"] == "order_intent")
    assert engine._broker.position is None
    receipt = intent["received_timestamp_ms"]

    engine.observe_paper_quote(
        received_at_ms=receipt + 1000,
        venue_event_ms=receipt + 500,
        bid=100.0,
        ask=100.1,
        source_id="depth-1",
    )
    fill = next(record for record in records if record["kind"] == "fill")
    quote = next(record for record in records if record["kind"] == "paper_quote_observed")
    assert quote["payload"]["source_id"] == "depth-1"
    assert quote["payload"]["order_received_at_ms"] == receipt
    assert fill["event_timestamp_ms"] == receipt + 1000
    assert fill["event_timestamp_ms"] >= intent["received_timestamp_ms"]
    opened = next(event for event in events if event["event_type"] == "TRADE_OPENED")
    assert opened["timestamp_ms"] == fill["event_timestamp_ms"]
    assert engine._broker.position is not None
    account = next(record for record in reversed(records) if record["kind"] == "account_snapshot")
    assert account["event_timestamp_ms"] == receipt + 1000
    assert account["payload"]["equity"] == pytest.approx(engine._broker.equity)

    protection = next(
        record
        for record in records
        if record["kind"] == "audit" and record["payload"].get("type") == "protection_confirmed"
    )
    with pytest.raises(ValueError, match="precedes protection"):
        engine.observe_paper_quote(
            received_at_ms=receipt + 2_000,
            venue_event_ms=protection["received_timestamp_ms"] - 1,
            bid=94.0,
            ask=94.1,
            source_id="depth-before-protection",
        )
    assert engine._broker.position is not None

    engine._finalize()
    assert engine._broker.position is not None
    assert engine.checkpoint["engine_state"]["terminal_policy"] == "retain_observed_paper_position"
    assert statuses[-1]["terminal_policy"] == "retain_observed_paper_position"
    assert statuses[-1]["open_position"] is not None
    assert not any(
        record["kind"] == "fill" and record["payload"].get("role") == "flatten"
        for record in records
    )


def test_observed_quote_paper_close_event_uses_exit_quote_timestamp(monkeypatch):
    monkeypatch.setattr(
        "koval.engine.live_engine.assemble_from_graph", lambda graph: _SingleEntryStrategy()
    )
    records, events = [], []
    engine = LiveEngine(
        ...
        session_id="causal-close-test",
        on_record=records.append,
        on_event=events.append,
    )
    engine._process_bar(np.array([0, 100.0, 101.0, 99.0, 100.0, 10.0]))
    receipt = next(r["received_timestamp_ms"] for r in records if r["kind"] == "order_intent")
    engine.observe_paper_quote(
        received_at_ms=receipt + 1000,
        venue_event_ms=receipt + 500,
        bid=100.0,
        ask=100.1,
        source_id="depth-entry",
    )
    protection = next(
        r["received_timestamp_ms"]
        for r in records
        if r["kind"] == "audit" and r["payload"].get("type") == "protection_confirmed"
    )
    exit_ms = max(receipt + 3000, protection + 1000)
    engine.observe_paper_quote(
        received_at_ms=exit_ms,
        venue_event_ms=exit_ms - 500,
        bid=94.0,
        ask=94.1,
        source_id="depth-exit",
    )
    closed = next(event for event in events if event["event_type"] == "TRADE_CLOSED")
    assert closed["timestamp_ms"] == exit_ms


def test_observed_quote_paper_replays_identical_economic_journal(monkeypatch):
    monkeypatch.setattr(
        "koval.engine.live_engine.assemble_from_graph", lambda graph: _SingleEntryStrategy()
    )
    ...
    original = LiveEngine(
        {"blocks": [], "connections": []},
        config,
        session_id="observed-replay",
        on_record=original_records.append,
    )
    bar = np.array([0, 100, 101, 99, 100, 10], dtype=float)
    original._process_bar(bar)
    receipt = next(
        record["received_timestamp_ms"]
        for record in original_records
        if record["kind"] == "order_intent"
    )
    original.observe_paper_quote(
        received_at_ms=receipt + 1_000,
        venue_event_ms=receipt + 500,
        bid=100.0,
        ask=100.1,
        source_id="depth-1",
    ...
    replayed = LiveEngine(
        ...
        config,
        session_id="observed-replay",
        on_record=replayed_records.append,
    )
    replayed._process_bar(bar)
    quote = next(r["payload"] for r in original_records if r["kind"] == "paper_quote_observed")
    replayed.apply_observed_paper_quote(quote)
    replayed._finalize()

    def projection(records):
        return [(r["kind"], r["event_timestamp_ms"], r["payload"]) for r in records]

    assert projection(replayed_records) == projection(original_records)
